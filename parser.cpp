...
bool SimpleStmt(istream &in, int &line)
{
    ...
    if (t.GetToken() == IDENT)
    {
        ...
        cout << "Assignment statement in a Simple If statement.\n";
    }
...
bool SimpleIfStmt(istream &in, int &line)
{
    ...
    {
        ParseError(line, "Missing Simple Stmt");
        return false;
    }
    return true;
}
...
bool BlockIfStmt(istream &in, int &line)
{
    ...
    {
        cout << "End of Block If statement at nesting level " << level-- << "\n";
    }
    return true;
}
...
bool isDeclared(string var, int line)
{
    if (defVar.find(var) != defVar.end())
    {
...
bool VarList(istream &in, int &line)
{
    ...
    if (t.GetToken() == ASSOP)
    {
        ...
        {
            defVar[var] = true;
            cout << "Initialization of the variable " << var << " in the declaration statement.\n";
            t = Parser::GetNextToken(in, line);
        }
    }
    else
...
bool Decl(istream &in, int &line)
{
    ...
    if (length.size() > 0)
        cout << "Definition of Strings with length of " << length << " in declaration statement.\n";
    return true;
}
